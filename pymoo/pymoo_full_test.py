...
class MyProblem(Problem):
    ...
    def __init__(self, config):
        ...
        super().__init__(
            ...
            xl=np.array([0.4 for i in range(0, 7)] + [0 for i in range(7, 24)] + [0 for i in range(24, config[
                "Q"].size * 24 + config["ita_num"])]),
            xu=np.array([1 for i in range(0, 7)] + [0.4 for i in range(7, 24)] + [1 for i in range(24, config[
                "Q"].size * 24 + config["ita_num"])]),
...
config = {}
config["Q"] = np.array([50]) * 1000  # 冷机额定功率  单位：kw
config["e"] = e = 1000000  # 等式约束的允许误差
...
handcraft_solution = [0 for i in range(41)]
handcraft_solution[1] = 1
handcraft_solution[2] = 1
handcraft_solution[3] = 1
handcraft_solution[4] = 1
handcraft_solution[5] = 0.5
handcraft_solution[24:-1] = [1 for i in range(17)]
problem.plan_printer(handcraft_solution)
